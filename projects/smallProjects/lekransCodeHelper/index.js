...
const colorize = txt => `${currentFgColor}${currentBgColor}${txt}${STATE.Reset}`;
...
const header = (s) => {
  ...
  console.log('');
  console.log(`${config.HEADER_TEXT_COLOR}${center(s).toUpperCase()}${COLORS.FgDefault}`);
  console.log('');
  console.log(`${config.HEADER_BORDER_COLOR}${sHash}${COLORS.FgDefault}`);
};
...
const footer = (s) => {
  ...
  const fillCount = slength - (shortHash.length * 2 + s.length + spacing.length * 2);
