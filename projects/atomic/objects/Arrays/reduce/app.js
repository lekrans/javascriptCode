...
log.log(
  ary.reduce((total, currentValue, currentIndex, arr) => total + currentValue),
);

log.subHeader2(`and now an array of objects wich we want to reduce to the sum of the ages
NOTE that we need to check if currentIndex === 1 because the FIRST total we get will be the
FIRST element.. so we need to use THAT elements age and add it to the currentvalue
otherwise we will add the age of the second element to the string version of the first object`);
const objAry = [
  { name: 'Micke', age: 54 },
  { name: 'Jesper', age: 32 },
  { name: 'Stefan', age: 46 },
...
log.log(
  objAry.reduce((total, currentValue, currentIndex, arr) => {
    if (currentIndex === 1) {
